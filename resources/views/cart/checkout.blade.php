                                ...
                                <ul class="list-group mb-3">
                                    @foreach (Cart::getContent() as $item)
                                        <li class="list-group-item d-flex justify-content-between lh-condensed">
                                            ...
                                            <span
                                                class="text-muted">$ {{number_format($item->price * $item->quantity) }}</span>
                                        </li>
                                    @endforeach
...
<!--Boton realizar pedido-->
                                ...
                                <div class="input-group">
                                    @if(auth()->check())
                                        @auth
                                            <a class="btn btn-primary btn-xs btn-block btn"
                                               href="/order/create">Realizar
                                                pedido</a>
                                        @endauth
                                    @else
                                    ...
                                    <div class="d-flex justify-content-end d-block mt-2">
                                        <a class="btn btn-link" href="{{ route('welcome') }}"><- Volver a la tienda</a>
                                    </div>
                                </div>
                            </div>
<!-- Fin resumen carrito -->
...
<!--Aumentar o disminuir unidades-->
                                                    <td class="text-center">
                                                        ...
                                                        <form action="{{ route('cart.upitem') }}" method="POST">
                                                            ...
                                                            <input type="submit" name="btn" class="btn btn-block btn-sm btn-primary mb-2 boton" value="+">
                                                        </form>
